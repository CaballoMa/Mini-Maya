...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifBindMats(-1), unifCurrOverallTransMats(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifCurrOverallTransMats = context->glGetUniformLocation(prog, "u_TransMats");
}
...
void ShaderProgram::setBindMatrix(const std::vector<glm::mat4> &bind)
{
    ...
    if(unifBindMats != -1)
    {
        context->glUniformMatrix4fv(unifBindMats,bind.size(),GL_FALSE,&bind[0][0][0]);
    }
}
...
void ShaderProgram::setTransMatrix(const std::vector<glm::mat4> &trans)
{
    ...
    if(unifCurrOverallTransMats != -1)
    {
        context->glUniformMatrix4fv(unifCurrOverallTransMats, trans.size(), GL_FALSE, &trans[0][0][0]);
    }
}
